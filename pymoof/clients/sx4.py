...
from pymoof.profiles.sx4 import SX4Profile
...
class SX4Client:
    """
    A wrapper around a bleak client that allows bluetooth communication with a Vanmoof S4 and X4.
    ...
    def __init__(
        ...
        self._bike_profile = SX4Profile(key, user_key_id)
    ...
    async def _write(self, characteristic_uuid, data: bytes) -> None:
        payload = self._bike_profile.build_encrypted_payload(data)
    ...
    async def set_bell_tone(self, bell_tone: BellTone) -> None:
        ...
        **Must be authenticated to call**
        ...
        :param bell_tone: The type of bell tone to use.
            See ``pymoof.clients.sx4.BellTone`` for a list of valid bell tones.
    ...
    async def set_lock_state(self, state: LockState) -> None:
        ...
        **Must be authenticated to call**
        ...
        :param state: The lock state to use. See
            ``pymoof.clients.sx4.LockState`` for a list of lock states.
        :raises ``bleak.exc.BleakError``: if the client is not authenticated.
        """
        await self._write(
    ...
    async def play_sound(self, sound: Sound, count: int = 1):
        ...
        **Must be authenticated to call**
        ...
        :param sound: The sound to use. See ``pymoof.clients.sx4.Sound`` for a list of valid sounds.
    ...
    async def get_lock_state(self) -> LockState:
        ...
        **Must be authenticated to call**
        ...
        :return: A ``pymoof.clients.sx4.LockState`` of the lock state of the bike.
        """
        result = await self._read(
            self._bike_profile.Defense.LOCK_STATE,
        )
